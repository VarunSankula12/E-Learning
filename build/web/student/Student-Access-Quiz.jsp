	...
	<section class="hero-wrap hero-wrap-1" style="background-image: url('images/bg_2.jpg');">
		...
		<div class="text-white">
			<p class="breadcrumbs"><span class="mr-2"><a href="../index.html">Home <i class="fa fa-chevron-right"></i></a></span>
				...
				<span class="mr-2"><a href="Student-Quiz.jsp">Quiz <i class="fa fa-chevron-right"></i></a></span><span class="mr-2"><a href="index.html">Student-Access-Java-Quiz <i></i></a></span></p>
		</div>
        ...
        <%
                ...
                ResultSet result=instructorDao.InstructorCourses(b);
                if(result.next()){
                
                b.setName(result.getString("instructor_name"));
                ResultSet rs=instructorDao.DisplayQuiz(b);
                if(rs.next()){
                   String question[]=rs.getString("questions").split(",");
		...
		<div class="row justify-content-center pb-4">
			<div class="col-md-12 heading-section text-center ftco-animate bg-primary">
				<span class="subheading"></span>
			  <h2 class="mb-4"><%= coursename%></h2>
		  </div></div>
	
          <div class="container mt-5">
            <h1 class="mb-4">QUIZ</h1>
            <form id="quizForm" action="Student-result.jsp" method="post">
    <input type="hidden" name="questionCount" value="<%= question.length-1%>">
                  ...
                  <div class="form-group">
                      <%
                          ...
                          int increement=question.length-2;
                          int qcount=question.length-1;
                          int optioncount=0;
                          int val=options.length;
                          String[] valid=new String[qcount];
                          ...
                          optioncount=options.length-(qcount+i);
                      %>
            <label for="question1"><%= question[increement] %></label>
            
            <select class="form-control" id="question" name="selectedOptions<%= i %>">
                <option>Select an answer</option>
                <%
                    for(int j=1;j<=4;j++){
                %>
                <option value="<%=j%>"><%= options[optioncount] %></option>
                <%
                    optioncount=optioncount-qcount;
                    }%> </select><%increement--;
}
                %>
